...
const run = watt(function* (quest, query, isCursor) {
...
Goblin.registerQuest(goblinName, 'query', function* (quest, query, args) {
  ...
  return yield q.getCursor();
});

//TODO: deprecate this one, refactor all code base for using
//      cursors...
Goblin.registerQuest(goblinName, 'query-sync', function* (quest, query, args) {
  const _query = new Function('return ' + query.toString())();
  const q = _query(r, ...args);
  return yield run(quest, q, false);
});
