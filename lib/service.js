...
Goblin.registerQuest(goblinName, 'start-listen-changes', function*(
  ...
  try {
    ...
    let questArguments = {id: goblinId, change};
    if (questArgs) {
      Object.keys(questArgs).forEach(function(key) {
        questArguments[key] = questArgs[key];
      });
    }

    quest.cmd(onChangeQuest, questArguments);
...
Goblin.registerQuest(goblinName, 'ensure-index', function*(quest, table, next) {
  ...
  /* HACK: the indexes must be passed by arguments and retrieve via the
   *       goblin which is really using this table. I will remove this
   *       hack ASAP (MS).
   */
  if (table === 'missionOrder') {
    indexes.push({
      index: 'billingCustomerId_createdAt',
      fields: [r.row('billingCustomerId'), r.row('meta')('createdAt')],
    });
    indexes.push({
      index: 'pickCustomerId_createdAt',
      fields: [r.row('pickCustomerId'), r.row('meta')('createdAt')],
    });
    indexes.push({
      index: 'dropCustomerId_createdAt',
      fields: [r.row('dropCustomerId'), r.row('meta')('createdAt')],
    });
  }

  if (table === 'document') {
    indexes.push({
      index: 'customerId_current',
      fields: [r.row('customerId'), r.row('current')],
    });
  }
