...
function applyContentIndex(q, contentIndex, filter, orderBy) {
  if (!contentIndex) {
    return q;
  }
...
Goblin.registerQuest(goblinName, 'getBaseList', function*(
  ...
  return yield run(quest, q('id'), next);
});
...
Goblin.registerQuest(goblinName, 'get-ids', function*(
  ...
  q = applyContentIndex(q, contentIndex, filter, orderby);
  if (range) {
...
Goblin.registerQuest(goblinName, 'count', function*(
  ...
  let q = r.table(table, {readMode: defaultReadMode});
  q = applyContentIndex(q, contentIndex, filter, orderBy).count();
  return yield run(quest, q, next);
});
...
Goblin.registerQuest(goblinName, 'ensure-secondary-index', function*(
  quest,
  table,
  name,
  path,
