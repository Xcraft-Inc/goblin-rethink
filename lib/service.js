...
Goblin.registerQuest(
  ...
  function*(
    ...
    const disposeSub = quest.sub(`*::${goblinId}.disposed`, function*() {
      disposeSub();
      yield quest.me.stopOnChanges({table, goblinId});
    });
...
Goblin.registerQuest(goblinName, 'start-listen-changes', function*(
  ...
  try {
    ...
    if (onErrorQuest) {
      let errorQuestArguments = {id: goblinId, err};
      if (errorQuestArgs) {
        Object.keys(errorQuestArgs).forEach(function(key) {
          errorQuestArguments[key] = errorQuestArgs[key];
        });
      }

      quest.cmd(onErrorQuest, errorQuestArguments);
    } else {
      quest.log.err(err);
    }
  }
