...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  host = _host[0];
  const port = _host[1] ? parseInt(_host[1]) : 28015;
  yield r.connectPool({
    db: database,
    servers: [{host, port}],
  ...
  yield quest.me.ensureDatabase();

  yield waitForAvailability(quest, database);

  const Piscina = require('piscina');
  // Main advantage of worker pool:
  //- multiple rethinkdb connections
  //- parallel driver results ser/deser can be cpu intensive
  quest.log.dbg('Init worker pool...');
  const piscina = new Piscina({
    filename: path.join(__dirname, 'worker.js'),
    workerData: {database, host, port},
    idleTimeout: 2000,
  });
  quest.goblin.setX('pool', piscina);
...
Goblin.registerQuest(goblinName, 'getConfiguration', function* (quest) {
...
Goblin.registerQuest(goblinName, 'ensure-database', function* (quest) {
  const db = quest.goblin.getX('db');
  const dbList = yield r.dbList().run();
  if (dbList.indexOf(db) === -1) {
    yield r.dbCreate(db).run();
  }
  //////////////////////////
  //ensure reader user exist
  const userId = `${db}-reader`;
  const reader = yield r.db('rethinkdb').table('users').get(userId).run();
  if (!reader) {
    yield r
      .db('rethinkdb')
      .table('users')
      .insert({id: userId, password: db})
      .run();
    yield r
      .db(db)
      .grant(userId, {read: true, write: false, config: false})
      .run();
  }
});
...
Goblin.registerQuest(goblinName, 'collect-stats', function (quest, database) {
  ...
  const setX = quest.goblin.setX.bind(quest.goblin);
  const collect = watt(function* () {
    ...
    setX('clusterStats', stats.query_engine);
  });

  quest.goblin.setX('clusterStatsInterval', setInterval(collect, 10000));
});

Goblin.registerQuest(goblinName, 'delete', function (quest) {
  const interval = quest.goblin.getX('clusterStatsInterval');
  if (interval) {
