...
module.exports = config => {
  ...
  Goblin.registerQuest (goblinName, 'create', function* (
    ...
    const skipEntity = yield quest.warehouse.has ({path: entityId});
    if (skipEntity) {
      console.log ('Sub workitem loading ', entityId);
      i.use (entityId);
      entity = yield quest.warehouse.get ({path: entityId});
    } else {
      ...
      if (!entity) {
        ...
        if (!entity) {
          ...
          if (!existing) {
            console.log ('requesting entity state ', entityId);
            entity = yield e.get ();
          } else {
            console.log ('using warehouse entity state', entityId);
            entity = existing;
          }
        }
      }
      ...
      for (const ref in entity.meta.references) {
        if (common.referenceUseArity (entity.meta.references[ref])) {
          ...
          for (const rId of entity[ref]) {
            ...
            const mustUse = yield quest.warehouse.has ({path: entityEditorId});
            if (mustUse) {
              i.use (entityEditorId);
              i.use (rId);
            } else {
          ...
          if (entity[ref] !== null) {
            ...
            const mustUse = yield quest.warehouse.has ({path: editorId});
            if (mustUse) {
              i.use (editorId);
              i.use (entity[ref]);
            } else {
              yield quest.create (editorId, {
                id: editorId,
                desktopId,
                entityId: entity[ref],
              });
  ...
  Goblin.registerQuest (goblinName, 'get-entity', function* (quest, entityId) {
    const desktopId = quest.goblin.getX ('desktopId');
    return yield common.getEntityQuest (quest, desktopId, entityId);
  });
