      ...
      for (const ref in entity.meta.references) {
        if (common.referenceUseArity (entity.meta.references[ref])) {
          ...
          for (const rId of entity[ref]) {
            ...
            const skip = yield quest.warehouse.has ({path: entityEditorId});
            if (skip) {
              continue;
            }
          ...
          const skip = yield quest.warehouse.has ({
            path: `${type}-plugin@${quest.goblin.id}`,
          });
          if (skip) {
            continue;
          }
          ...
          if (entity[ref] !== null) {
            const skip = yield quest.warehouse.has ({
              path: `${type}-workitem@${quest.goblin.id}`,
            });
            if (skip) {
              continue;
            }
            quest.create (`${type}-workitem`, {
              id: `${type}-workitem@${quest.goblin.id}`,
              desktopId,
              entityId: entity[ref],
              skipEntity: true,
            });
