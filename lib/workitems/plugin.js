...
module.exports = config => {
  ...
  Goblin.registerQuest(goblinName, 'create', function(
    ...
    quest.goblin.defer(
      quest.sub(`*::${forEntity}.collection-updated`, (err, msg) => {
        if (msg.data.type === type) {
          quest.me.updateCollection({entityIds: msg.data.entityIds});
        }
      })
    );

    quest.goblin.defer(
      quest.sub(`*::${forEntity}.(val|ref)-moved`, (err, msg) => {
    ...
    quest.goblin.defer(
      quest.sub(`*::${forEntity}.(val|ref)-added`, (err, msg) => {
        if (extractType(msg) === type) {
          quest.me.onAdd({
    ...
    quest.goblin.defer(
      quest.sub(`*::${forEntity}.(val|ref)-removed`, (err, msg) => {
        if (extractType(msg) === type) {
          quest.me.remove({
            entityId: extractEntityId(msg),
            skipRemove: true,
  ...
  Goblin.registerQuest(goblinName, 'on-add', function*(
    quest,
    entityId,
    entity,
    beforeId
  ...
  Goblin.registerQuest(goblinName, 'add', function(quest, beforeId) {
    const forEntityId = quest.goblin.getX('forEntity');
    const newEntityPayload = quest.goblin.getX('newEntityPayload');

    const i = quest.openInventory();
    const entityAPI = i.getAPI(forEntityId);
    const method = methodBuilder('add-new');
    entityAPI[method]({
