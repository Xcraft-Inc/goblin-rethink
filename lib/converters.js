...
function jsToCanonicalTime (time) {
  return (
...
function canonicalTimeToJs (time) {
...
function jsToCanonicalDate (date) {
  return (
...
function canonicalDateToJs (date) {
...
function isCanonicalDate (canonicalDate) {
  if (
    !canonicalDate ||
...
function splitDate (canonicalDate) {
  if (
    !canonicalDate ||
    canonicalDate.length !== 10 ||
    canonicalDate[4] !== '-' ||
...
function splitTime (canonicalTime) {
  if (
    !canonicalTime ||
...
function getDisplayedDate (canonicalDate, format) {
...
function getDisplayedTime (time, format) {
...
function parseEditedDate (editedDate, defaultCanonicalDate) {
...
function parseEditedTime (editedTime, defaultCanonicalTime) {
...
function getNowCanonicalTime () {
  return jsToCanonicalTime (new Date (Date.now ()));
}

function getNowCanonicalDate () {
