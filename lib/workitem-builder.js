...
const searchTemplate = config => {
  ...
  const logicHandlers = {
    ...
    'load-index': (state, action) => {
      const docs = action.get ('docs');
      state = state.set ('list', docs);
      return state;
    },
  };
  ...
  Goblin.registerQuest (goblinName, 'create', function* (quest, desktopId) {
    ...
    let count = 0;
    if (list) {
      count = yield r.count ({table: list});
      const listIds = yield r.getBaseList ({table: list});
  ...
  Goblin.registerQuest (goblinName, 'load-index', function* (quest, index) {
    const cTo = quest.goblin.getX ('to');
    const cFrom = quest.goblin.getX ('from');

    if (index >= cFrom && index <= cTo) {
      return;
    }
    ...
    const from = index;
    const to = index + 100;
    quest.goblin.setX ('from', from);
    quest.goblin.setX ('to', to);
    const documents = listIds.slice (from, to);
    const docs = yield r.getAll ({table: list, documents});
    const rows = {};
  ...
  Goblin.registerQuest (goblinName, 'init-list', function* (quest) {
    ...
    quest.dispatch ('load-index', {docs: rows});
  });
