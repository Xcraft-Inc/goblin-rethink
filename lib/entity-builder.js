...
module.exports = config => {
  ...
  const logicHandlers = {
    ...
    updateMetaDataInfo: state => {
      ...
      return state.set (
        'meta.info',
        buildInfo (state.toJS (), new MarkdownBuilder ())
      );
    },
    backup: (state, action) => {
  ...
  Goblin.registerQuest (goblinName, 'create', function* (
    ...
    if (entity) {
      ...
      entityMeta.set (entity, type, references, initialStatus, buildInfo);
    }
    ...
    if (entity.meta.status !== 'archived') {
      ...
      if (!isNew) {
        if (parentReference && !withoutReferences) {
          const rId = entity[parentReference];
          const useKey = `${rId}@${desktopId}`;
          const hasId = yield quest.warehouse.has ({path: rId});

          if (rId && !hasId && rId !== loadedBy) {
