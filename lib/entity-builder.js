...
module.exports = config => {
  ...
  Goblin.registerQuest (goblinName, 'handle-changes', function* (
    ...
    if (change.type === 'change') {
      ...
      const valToPatch = valuesProps.filter (p => props.indexOf (p) > 0);
      const newRefState = {};
      const newValState = {};
      ...
      for (const path of valToPatch) {
        newValState[path] = entity[path];
        delete entity[path];
      }

      // Make a preview patch (no persistance is needed)
      ...
      if (valToPatch.length > 0) {
        for (const path of valToPatch) {
          ...
          if (common.referenceUseArity (values[path])) {
            ...
            /*quest.evt ('remote-val-setted', {
              entity: newValState[path],
            });*/
          }
        }
      }
