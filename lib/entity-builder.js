...
const buildPeers = function* (quest, entity) {
  ...
  if (entity.forEntity && entity.forEntity !== null) {
    const parent = yield quest.me.getEntity ({entityId: entity.forEntity});
    peers.parent = parent;
  }
  return peers;
};
...
module.exports = config => {
  ...
  const logicHandlers = {
    ...
    'update-aggregate': (state, action) => {
      ...
      return state.set (entity.meta.rootAggregatePath, entity);
    },
  ...
  if (values) {
    ...
    for (const path in values) {
      ...
      if (common.referenceUseArity (val)) {
        ...
        valQuests[`add-new-${type}`] = function* (quest, payload) {
          if (!payload.forEntity) {
            payload.forEntity = quest.goblin.id;
          }
          const id = yield quest.me.addNewVal ({path, type, payload});
          return id;
        };
  ...
  Goblin.registerQuest (goblinName, 'hydrate-from-parent', function* (quest) {
    ...
    console.log ('hydrated from parent!');
  ...
  Goblin.registerQuest (goblinName, 'hydrate', function* (quest) {
    ...
    if (entity.meta.rootAggregateId !== quest.goblin.id) {
      console.log ('Update root aggregate');
      const i = quest.openInventory ();
      const rootAPI = i.getAPI (entity.meta.rootAggregateId);
      rootAPI.updateAggregate ({entity});
    }
  ...
  Goblin.registerQuest (goblinName, 'add-val', function* (quest, path, entity) {
    ...
    if (entity.forEntity !== quest.goblin.id) {
      entity.forEntity = quest.goblin.id;
    }
  ...
  Goblin.registerQuest (goblinName, 'persist', function (quest) {
    ...
    if (rootAggregateId !== newEntityJs.id) {
      ...
      console.log ('NESTED SAVE:');
      console.dir (newEntityJs);
      r.setIn ({
        table: rootType,
        documentId: rootAggregateId,
        path: rootAggregatePath,
