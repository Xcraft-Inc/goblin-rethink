...
module.exports = config => {
    ...
    for (const path in values) {
      ...
      if (common.referenceUseArity (val)) {
        ...
        valQuests[`add-new-${type}`] = function* (quest, payload) {
          ...
          yield quest.me.addNewVal ({path, type, payload});
        };
        ...
        valQuests[`remove-${type}`] = function (quest, entityId) {
          quest.me.removeVal ({path, entityId});
        };
        ...
        //Entity case
        valQuests[`set-${path}`] = function (quest, entity) {
          quest.me.setVal ({path, entity});
        };
      }
    }
  ...
  Goblin.registerQuest (goblinName, 'remove-val', function* (
    ...
    quest.do ();
    quest.evt ('val-removed', {entityId});
    ...
    const i = quest.openInventory ();
    const value = i.getAPI (entityId);
    value.delete ({hard: true});
  });
