...
const fetchPeers = function* (quest, peers, entity, references, path) {
  ...
  if (ref.endsWith ('[]')) {
    for (const rId of entity[path]) {
      const refAPI = i.use (rId);
      const peer = yield refAPI.get ();
      if (peer) {
        ...
        console.log ('Fetch fail ready for: ', rId);
      }
    }
  } else {
    //Entity case
        ...
        for (const path in references) {
          ...
          if (ref.endsWith ('[]')) {
            for (const rId of entity[path]) {
              ...
              //REHYDRATE
              refSubs[rId].push (
                quest.sub (`${rId}.changed`, quest.me.hydrate)
              );
            }
          } else {
            //Entity case
            ...
            //RE-HYDRATE
            refSubs[rId].push (quest.sub (`${rId}.changed`, quest.me.hydrate));
          }
        }
      ...
      if (refToPatch.length > 0) {
        for (const path of refToPatch) {
          ...
          if (references[path].endsWith ('[]')) {
            const refType = references[path].split ('[')[0];
  ...
  if (descriptor) {
    Goblin.registerQuest (goblinName, 'describe', function* (
      ...
      if (isGenerator (descriptor)) {
        desc = yield* descriptor (quest, entity, peers, next);
      } else {
        desc = descriptor (quest, entity, peers);
      }
      if (desc === undefined) {
        throw new Error (
          'Bad descritor for ',
