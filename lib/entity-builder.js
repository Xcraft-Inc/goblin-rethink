...
const immDiff = require ('immutablediff');
const immPatch = require ('immutablepatch');
...
module.exports = config => {
  ...
  const logicHandlers = {
    ...
    patch: (state, action) => {
      const ops = action.get ('ops');
      return new Goblin.Shredder (immPatch (state.state, ops));
    },
  ...
  Goblin.registerQuest (goblinName, 'create', function* (
    ...
    //LISTEN ENTITY FLOW
    const buildEntityFlowListener = (quest, verb) => {
      quest.goblin.defer (
        quest.sub (`*.${quest.goblin.id}.${verb}-requested`, quest.me[verb])
      );
    };

    buildEntityFlowListener (quest, 'publish');
    buildEntityFlowListener (quest, 'restore');
    buildEntityFlowListener (quest, 'archive');
              ...
              for (const rId of entity[path]) {
                ...
                if (rId && rId !== loadedBy) {
                  try {
                    ...
                    if (err.code === 'EENTITY_NOT_FOUND') {
                      // Remove ref from collection
                      entity[path] = entity[path].splice (
                        entity[path].indexOf (rId),
                        1
                      );
                    } else {
                      throw err;
  ...
  Goblin.registerQuest (goblinName, 'handle-changes', function (quest, change) {
    if (change.type === 'change') {
      ...
      let current = quest.goblin.getState ().del ('private.backup');
      if (current.equals (newState)) {
        return; // nothing to patch...
      }

      //Retreive references and values props
      ...
      if (values) {
        valuesProps = Object.keys (values);
      }
      const refToNotify = referencesProps.filter (p => props.indexOf (p) > 0);
      const valToNotify = valuesProps.filter (p => props.indexOf (p) > 0);

      //Patch current entity state with new one
      const ops = immDiff (current.state, newState.state);
      quest.dispatch ('patch', {ops});
      current = quest.goblin.getState ();

      //Notify UI activities of changes
      for (const path of refToNotify) {
        ...
        if (common.referenceUseArity (references[path])) {
          ...
          const currentArray = currentRefs.toArray ();
          quest.evt ('collection-updated', {
            type: refType,
            entityIds: currentArray,
          });
        } else {
          quest.evt ('pointer-updated', {
      ...
      for (const path of valToNotify) {
        const currentValues = current.get (path);
        if (common.referenceUseArity (values[path])) {
          const currentArray = currentValues.toArray ();
          const valType = common.getReferenceType (values[path]);
          quest.evt ('collection-updated', {
            type: valType,
            entityIds: currentArray,
          });
  ...
  Goblin.registerQuest (goblinName, 'persist', function (quest) {
    ...
    if (rootAggregateId !== newEntityJs.id) {
      ...
      delete newEntityJs.meta.persistedFromDesktopId;
      r.setIn ({
        table: rootType,
        documentId: rootAggregateId,
        path: rootAggregatePath,
  ...
  const buildEntityFlowPropagator = (verb, evt) => {
    Goblin.registerQuest (goblinName, `${verb}-entity`, function* (
      quest,
      next
    ) {
      const document = quest.goblin.getState ().toJS ();
      quest.defer (quest.sub (`${document.id}.${evt}`, next.parallel ()));
      quest.evt (`${document.id}.${verb}-requested`);
      //cascade sub-documents
      const subRollback = subDoc => {
        for (const path in subDoc.meta.values) {
          for (const entityId of Object.keys (subDoc[path])) {
            const toRollback = subDoc.private[path][entityId];
            if (toRollback) {
              subRollback (toRollback);
              quest.defer (
                quest.sub (`${toRollback.id}.${evt}`, next.parallel ())
              );
              quest.evt (`${toRollback.id}.${verb}-requested`);
            }
          }
        }
      };
      subRollback (document);
      yield next.sync ();
  ...
  Goblin.registerQuest (goblinName, 'publish', function (quest) {
    ...
    //if root draft is untouched -> delete document
