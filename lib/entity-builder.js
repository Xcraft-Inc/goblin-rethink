...
module.exports = config => {
  ...
  Goblin.registerQuest (goblinName, 'create', function* (
    ...
    yield quest.me.bootReferences ({entity, load: !isNew, loadedBy});
    ...
    if (change.type === 'change') {
      ...
      if (
        ...
        if (current.equals (newState)) {
          ...
          const referencesProps = Object.keys (references);
          const refToPatch = referencesProps.filter (
            p => props.indexOf (p) > 0
          );
          const newRefState = {};
              ...
              for (const path of refToPatch) {
                ...
                if (references[path].endsWith ('[]')) {
                  ...
                  for (const rId of toAdd) {
                    quest.evt ('remote-ref-added', {entityId: rId});
                  }
                  for (const rId of toRemove) {
                    quest.evt ('remote-ref-removed', {entityId: rId});
                  }
                } else {
  ...
  Goblin.registerQuest (goblinName, 'boot-references', function* (
    quest,
    entity,
    load,
    loadedBy
    ...
    if (entity.meta.status !== 'archived') {
      yield quest.me.rebuildReferencesSubscriptions ({entity});
          ...
          for (const path in references) {
            ...
            if (ref.endsWith ('[]')) {
              for (const rId of entity[path]) {
                ...
                if (!quest.canUse (useKey) && rId !== loadedBy) {
  ...
  Goblin.registerQuest (
    goblinName,
    'rebuild-references-subscriptions',
