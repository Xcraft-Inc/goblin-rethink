...
function builder(config) {
  ...
  Goblin.registerQuest(goblinName, 'add-ref', function*(
    ...
    refSubs[entityId].push(quest.sub(`${entityId}.changed`, quest.me.hydrate));

    quest.goblin.setX('refSubs', refSubs);
    quest.evt('ref-added', {entityId, beforeId});
  ...
  Goblin.registerQuest(goblinName, 'add-val', function*(
    ...
    quest.goblin.setX('valSubs', valSubs);
    quest.evt('val-added', {entity, beforeId});
  ...
  Goblin.registerQuest(goblinName, 'move-ref', function(
    ...
    quest.do();
    quest.evt('ref-moved', {entityId, beforeEntityId});
    quest.evt('changed');
    quest.me.persist();
  });
  ...
  Goblin.registerQuest(goblinName, 'move-val', function(
    ...
    quest.do();
    quest.evt('val-moved', {entityId, beforeEntityId});
    quest.evt('changed');
    quest.me.persist();
  });
          ...
          if (subDoc[path]) {
            for (const entityId of subDoc[path]) {
              ...
              if (entity) {
                ...
                quest.defer(quest.sub(`${entity.id}.${evt}`, next.parallel()));
                quest.evt(`${entity.id}.${verb}-requested`);
              }
            }
          }
        }
