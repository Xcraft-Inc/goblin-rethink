    ...
    for (const path in references) {
      ...
      if (ref.endsWith ('[]')) {
        for (const rId of entity[path]) {
          const entity = yield quest.warehouse.get ({path: rId});
          if (!peers[entity.meta.table]) {
            peers[entity.meta.table] = {};
          }
          peers[entity.meta.table][entity.id] = entity;
        }
      } else {
        //Entity case
...
module.exports = config => {
  ...
  const logicHandlers = {
    ...
    compute: (state, action) => {
      return state.set ('sums', action.get ('sums'));
    },
  ...
  Goblin.registerQuest (goblinName, 'create', function* (
    ...
    if (entity) {
      ...
      entityMeta.set (entity, table, references, initialStatus, buildInfo);
    }
        ...
        for (const path in references) {
          ...
          if (ref.endsWith ('[]')) {
            for (const rId of entity[path]) {
              ...
              //RE-COMPUTE
              if (computer) {
                refSubs.push (quest.sub (`${rId}.changed`, quest.me.compute));
              }
            }
          } else {
            //Entity case
    ...
    if (computer && isNew) {
      ...
      if (isGenerator (computer)) {
        sums = yield* computer (quest, entity, peers, next);
      } else {
        sums = computer (quest, entity, peers);
      }
      quest.dispatch ('compute', {sums});
